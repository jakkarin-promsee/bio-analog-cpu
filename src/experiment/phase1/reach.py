"""Phase 1 shared reachability — the representational LIMIT of the atom.
...
def gaussian(X1, X2, sx=0.5, sy=None, cx=0.0, cy=0.0):
    """A parametrized gaussian bump. Width (sx, sy) sets the difficulty: small = sharp/hard (a spike),
    large = wide/easy (≈ flat). Used to sweep *shape difficulty* (exp-5)."""
    sy = sx if sy is None else sy
    return np.exp(-((X1 - cx) ** 2 / (2 * sx ** 2) + (X2 - cy) ** 2 / (2 * sy ** 2)))
...
def fit_to(Tn, X1, X2, n_restart=12, seed=0, ceiling=None, scale_free=False):
    ...
    `ceiling` (rung-1): bound |w| <= ceiling — the representational limit UNDER the weight cap.
    `scale_free`: regress Tn onto [model_output, 1] each step, so AMPLITUDE/gain is factored out and only
    SHAPE is fit. Use it under a ceiling — otherwise the residual just measures gain collapse, not shape.
    Returns (best_w, norm_resid)."""
    ...
    def resid(w):
        ...
        if not scale_free:
            return Z - t
        A = np.column_stack([Z, np.ones_like(Z)])          # absorb amplitude + offset -> shape-only residual
...
def fit_target(name, n_grid=21, n_restart=12, seed=0, domain=(-1.0, 1.0), ceiling=None):
    """Best-fit out0 to a named target over an n_grid^2 grid (wraps `fit_to`)."""
    g = np.linspace(domain[0], domain[1], n_grid)
    X1, X2 = np.meshgrid(g, g)
    Tn, _, _ = normalize(target_surface(name, X1, X2))
    return fit_to(Tn, X1, X2, n_restart=n_restart, seed=seed, ceiling=ceiling)
