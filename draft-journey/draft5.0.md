# Draft 5.0 — Bio-Inspired Analog Neural Compute Architecture

> Canonical specification. v5.0.
> Supersedes draft4.1.
> Scope: Python simulation + math justification. Every choice must remain physically realizable in analog circuitry.

> **What's new vs 4.1:** The theoretical architecture is unchanged — §1–18 are stable. The simulation plan (§19) is rewritten from scratch into a detailed experimental campaign with: software infrastructure spec, methodological principles, task operationalization, hypothesis-to-experiment mapping, ablation matrices, hard exit criteria per phase, continuous invariant monitors, promotion/demotion criteria, negative-result protocols, and effort estimates. The architecture is locked; experiments now drive every remaining decision.
...
**If you read only one section, read §19** — the Simulation Plan is what makes this project move from theory to data. Everything in §1–18 is the spec; everything in §19 is the experimental campaign that tests it. The architecture is locked; experiments now drive every remaining decision.

If §19 is too long, the orientation reads are:

- **§12** — the central learning mechanism (attribution-based hierarchical diffusion).
- **§4.4** — how the architecture handles winner-take-all dynamics (spoiler: physics, not software).
- **§14** — analog robustness against PVT.
...
## 19. Simulation Plan — Detailed Experimental Campaign

This section is the operational plan for taking the architecture from spec to data. The theoretical work is locked (§1–18); from here, every remaining decision is driven by experiment, not intuition.
...
### 19.0 Methodological Principles
...
**Rule 4: Invariants checked everywhere, not in dedicated suites.** Loss conservation epsilon, dead-weight fraction, ceiling-saturation fraction, and clip rate are logged in _every_ experiment. There is no separate "loss conservation test" — every run validates it as a side effect. See §19.3.
...
**Rule 8: Architecture changes are decisions, not experiments.** If a phase's data suggests changing the architecture (e.g. promoting tanh from §20.5 to baseline), that decision goes through the §19.16 promotion criteria. Architecture doesn't drift silently.

### 19.1 Simulation Infrastructure
...
- Used in: Phase 7+ if everything else works. This is the "human eye mechanism" stretch goal from earlier brainstorms. **Will probably break first attempts** — that's expected.

### 19.3 Continuous Invariant Monitors
...
| **Dead-weight fraction**        | `(count Scaps with \|weight_cap\| < 1% of range) / total Scaps`      | < 5% by end of training  | Flag run; escalate to Path 0 in §19.16          |
| **Ceiling-saturation fraction** | `(count Scaps with momentum_SRAM ≥ 99% max) / total Scaps`           | < 5% by end of training  | Flag run; consider log-momentum (§20)           |
| **T_max clip rate**             | `(count Scaps hitting T_max in last forward) / total Scaps`          | < 20% by end of training | Flag run; auto-trigger Range Sensitivity (§5.4) |
...
### 19.4 Hypothesis-to-Experiment Map
...
If a hypothesis fails its pass criterion, that's a real result. See §19.17 negative-result protocols.

### 19.5 Phase 1 — Operator Sanity
...
### 19.6 Phase 2 — Single Ganglion (the central test)
...
**If Phase 2 fails:** see §19.17 negative-result protocols. Likely paths: enable Path 0 noise floor → retry; if still failing, escalate to Path 1 or 2.

### 19.7 Phase 3 — Ablation Suites
...
5 seeds × 5 cells × Task C = **25 runs**. Resolves Open Question 20.
...
### 19.8 Phase 4 — Column Composition

**Goal:** verify that hierarchical diffusion through Column-local buses works as designed, and that translate ALUs (learnable, §6.3) train.
...
- Convergence vs single-Ganglion baseline
- Loss conservation at Column level (additional level of monitoring)
- Translate ALU Scap weight distributions (do they actually train?)
...
### 19.9 Phase 5 — Multi-Parent Lobe DAG
...
### 19.10 Phase 6 — SpecialGeneralist

**Goal:** validate H2 (gated G-reuse > plain reuse > Reservoir-G).

**Three configurations × mask policies:**
...
### 19.11 Phase 7 — Limbic Loop with Two-Timescale Learning
...
- Recall accuracy on partial cues (Task E)
- Recurrence stability (does loop oscillate or collapse?)
- Noise amplification (variance of internal state over time)
- Convergence speed
...
### 19.12 Phase 8 — PVT Robustness

**Goal:** validate that the analog robustness mechanisms (§14) preserve performance under realistic PVT.

**Now and only now, add PVT realism to the simulator.**

**PVT components added:**

- Process variation: per-Scap multiplicative bias drawn from `N(1, σ_process)`, fixed per chip-run.
- Voltage variation: supply-rail droop modeled as activity-dependent local voltage decrease.
- Temperature variation: per-Lobe additive offset that grows with Lobe activity (warming) and decays slowly (cooling).
- Aging: very slow per-Scap drift (probably ignore for Phase 8, deferred to §20.16 SPICE).
...
- Performance drop vs Cell 8-1 baseline
- Loss conservation ε under perturbation
- Clip rate under PVT
- Calibration recovery time after thermal spike
...
### 19.13 Phase 9 — Synaptic Drift (conditional on H1, H2, H3 passing)

**Goal:** validate H4 (Synaptic Drift accelerates convergence).

**Activate only if Phases 2, 6, 7 succeed.**

**Configuration:**

- Add learnable 1D position SRAM per synapse in Commissure (§10.5).
- Position updates via ±1 per backward pass.
...
- ≥ 20% faster convergence with Drift vs without.
- If less, deprioritize and continue without.

**Effort estimate:** 2 weeks.

### 19.14 Phase 10 — Scale to 256 Ganglia per Region (conditional)

**Goal:** validate H5 (no weight LDR/STR) and characterize how everything behaves at scale.

**Activate only if Phase 7 succeeds.**

- Implement Route L1/L2 addressing (§15).
...
**Effort estimate:** 3 weeks (large simulation, slow runs).

### 19.15 Minimum Viable Falsification

**The fastest possible test of H1.** Before any phase, run this to fail fast if attribution doesn't work at all.

**Setup:** single Ganglion, Task A (XOR), full ideal configuration (Cell A from Phase 2), 1 seed.

**Expected runtime:** < 1 hour.

**Pass criterion:** loss decreases monotonically over training; final loss < 50% of initial loss.

**Fail action:** stop. Debug the operator layer (Phase 1) or the update equation (§4.8) before proceeding. Most likely failure: a sign error or measurement-direction bug.

This is a sanity gate, not a hypothesis test. It catches infrastructure bugs in hours instead of days.

### 19.16 Promotion / Demotion Criteria

When does the architecture change based on data?

**Promote to baseline (move from §20 Future Tracks to §1–18 spec):**

- Tanh activation (§20.5): if Phase 2 shows ReLU works _but_ Forward_Sign benefits at only 9/29 Scaps — and Phase 3 shows tanh would benefit all 29 with no convergence cost.
- Adam v*t per Scap (§20.4): if Phase 2 shows physical saturation alone produces > 5% ceiling-saturation \_or* fails to prevent winner-take-all (measured as: single Scap holds > 30% of total weight magnitude).
- Adaptive per-Scap weight decay (§20.2): if Phase 3 shows uniform decay produces > 10% dead-weight fraction even with floor-at-1 and residuals.
- Per-neuron residuals (§20.1): if Phase 3 residual ablation shows L1→L4 bypass insufficient (dead-weight > 5% with bypass on).
...
- ForwardSign → drop: if Phase 5c shows no measurable benefit on Task D (direction-conflict task). Saves 1 bit per Scap.

**Architectural changes require:**

- A documented Phase report showing the data.
- A draft revision (e.g. draft5.1) showing the change.
- The change goes through the §21 Protected List update process.

### 19.17 Negative Result Protocols

What to do when a phase fails its exit criteria.

**Phase 2 fails (H1 — attribution doesn't converge):**

1. Run minimum viable falsification (§19.15). Confirm not an infrastructure bug.
2. Enable Path 0 (noise floor on weight updates) — see §12.4. Re-run.
...
2. If still no benefit: drop ForwardSign per §19.16 demotion. Document the negative result.
...
2. Consider Ping-Pong substrate (§14.6) if Auto-Zeroing alone is insufficient.
...
### 19.18 Reproducibility Infrastructure
...
**Single biggest risk:** Phase 2 failure. If H1 doesn't converge after Path 0/1/2, the project's central architectural commitment needs revisiting. This is why the minimum viable falsification (§19.15) exists — to identify catastrophic failure in hours, not weeks.
