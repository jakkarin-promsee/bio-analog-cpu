# Draft 3.3 — Bio-Inspired Analog Neural Compute Architecture

> Working research design document, v3.3.
...
> **What's new vs 3.2:** A re-read pass surfaced several spec gaps. They are addressed here, mostly as clarifications rather than mechanism changes. The update equation (§6.5) is properly separated into two duties — `pulse_width` is the global update-time control (LR-like), `momentum SRAM` is the per-scap per-batch contribution accumulator (filled during forward passes). They are _not_ duplicate quantities. The time-to-threshold measurement (§7.4) is clarified — by the time the measurement is stored in momentum SRAM, it already represents contribution (not its inverse); the conversion is folded into the measurement circuit. Per-Column independent update buses are made explicit (§4.2, §10.7). Several known gaps (full Brainstem responsibilities, Phase 4 task definition, mask overlap policy) are flagged as placeholders without resolution, because resolving them prematurely would push past where the architecture is actually stable.
...
- **§5 (Attribution-Based Hierarchical Diffusion)** — the project's central mechanism. Honestly framed (since 3.2) as attribution-based learning, not approximate backprop.
- **§5.2 (What this actually is: attribution, not gradient)** — explains why this isn't a worse version of backprop; it's a different family of methods that fits the analog substrate naturally.
- **§6.5 (Backward-pass behavior)** — clarified in 3.3. `pulse_width` and `momentum SRAM` serve different duties; they are not duplicates.
- **§9 (SpecialGeneralist)** — gated reuse pattern. Replaces plain G-reuse.
- **§10 (Limbic Loop with two-timescale learning)** — Cortex-fast, Hippocampus-slow. Per-Column buses explicit (since 3.3).

If you read only one section, read §5. It's the structural decision that makes everything else click — and §6.5 in 3.3 makes the actual update equation precise for the first time.
...
- **Manages per-Column update gating** (clarified in 3.3). The Brainstem holds _separate_ update bus controls for Cortex, Hippocampus, and Commissure. Each is gated independently — Cortex's bus fires every clock, Hippocampus's every k clocks, Commissure's every M clocks. The "global bus" carries the loss; the _per-Column buses_ carry whether-to-actually-update-this-tick. This is what makes two-timescale learning work (§10).
- Manages **broadcast control signals**: `update_signal` (per-Column), `reset_history_signal`, `consolidate_signal` (for Hippocampus consolidation phases).
- Holds the **community SRAM map** that defines which Routes belong to which Column / Limbic region (set at boot).

> **Brainstem responsibilities are still being scoped.** (Placeholder for future work.) The list above is the minimum the Brainstem needs to do for the rest of the architecture to function as described. Several additional responsibilities are likely required — see §5.6 on Brainstem state for autoregressive loss (holding previous prediction to compare against current input as the label), and the open question on Hippocampus consolidation pulse amplification (§C4-equivalent). These are deliberately left underspecified at this stage; the architecture above the Brainstem is not stable enough to lock in the full controller spec.
...
If a future breakthrough lets us distribute the loss compute itself (e.g. each output Ganglion senses its own (prediction − label) locally), the Brainstem shrinks further. That is a §14 (Future Tracks) item, not a blocker.
...
1. **Dead weight collapse.** Because `ΔW_k` scales with `|a_k · W_k|` (via momentum), a weight that reaches zero magnitude receives zero update and stays dead forever. The "floor-at-1" rule on momentum SRAM (§6.3) partially mitigates this — but a _true_ zero weight is still trapped. The simulation must instrument zero-magnitude weight counts over training time. If a substantial fraction of scaps collapse, escalate first to Path 0 (noise floor on weight updates), then to Path 2 if needed.
...
**Four paths if simulation reveals a fatal problem:**

- _Path 0 (cheapest remediation):_ Add a tiny random noise floor to weight updates — every scap gets a small random perturbation each update, independent of measurement. This rescues dead weights without changing the measurement circuit. Try this first if dead-weight collapse appears; it's a one-line addition.
...
We are committing to Path 3 for the baseline. Path 0 is the first remediation if dead-weight specifically appears. Paths 1 and 2 are documented for deeper failures.
...
The share formula contains a division: `share = loss × (a_i / Σa_j)`. Division in digital binary is expensive. The architecture handles this **in the analog op-amp ALU domain, not in binary**:

- At each hierarchy level's ALU, normalization is performed as a continuous analog operation. The standard op-amp circuit for ratio-of-currents (a current divider, or an op-amp configured as a normalizing summing amplifier) takes the bank of measurement values as inputs and produces normalized outputs whose sum equals a fixed reference. No binary division step.
- The whole architecture leans on this: continuous analog math is the cheap path, binary math is the expensive path. Normalization is a place where that bet pays off.

When a module finishes its forward pass, it normalizes its measured contributions analog-style so they sum to a fixed reference (say, full-scale voltage `V_ref`). Then the share computation is:

```
share = loss × normalized_contribution_i
```

This is pure multiplication — cheap.

**By the time anything lands in a scap or in a level's distribution memory, it is already final-form, ready-to-use, no further division required.** The architecture enforces this consistently: each scap stores a single contribution-magnitude value that the update equation can consume directly.

The cost is per-module-per-forward-pass at each level (one analog normalization circuit invocation), not per-weight. At the Ganglion level, this is 29 measurements normalized once per forward pass — a single op-amp normalization bank handles it.

**Open architectural detail:** the precise op-amp topology for the normalizer is not yet specified. This is one of the gaps where the project still needs to do real circuit design (or, in the Python simulation phase, simply assume the operation is available). For draft3.3 it stays at the "we trust the analog substrate can do this" level; concrete circuit choice deferred to §14 future tracks.
...
> **Clarified in 3.3.** Earlier drafts wrote `weight_cap += pulse_width × momentum × direction` without distinguishing what the two factors _meant_. They are not duplicate signals. They serve different duties on different timescales:
>
> - **`momentum SRAM`** is the **per-scap contribution accumulator**. It is filled during forward passes — every forward pass adds the current measurement (which by §7.4 already represents `|a · W|` contribution, post-conversion) into the EMA. By update time, momentum SRAM holds the accumulated, smoothed importance of this specific scap across the batch.
> - **`pulse_width`** is the **global update-time control**, broadcast from the Brainstem (top-level) or from the next level up in the hierarchy after diffusion. It acts like a learning rate — same value for every scap on the bus, encoding "how much updating happens this round." At the simplest level it equals the loss magnitude `|L|`; with hierarchical diffusion it is the share that arrived at this Ganglion.
>
> Different duties, different filling cadence, no double-counting.
...
- The **PWM-encoded pulse** from the Ganglion's local bus (`pulse_width` = global update magnitude / share from above).
- Its own **momentum SRAM** (per-scap contribution accumulated across the batch).
...
- `pulse_width` is the same for every scap on this bus (global control, LR-like).
- `momentum` is per-scap (this scap's own contribution history).
- The minus sign moves the weight _against_ the loss direction (gradient-descent-flavor, though we are attribution-based not gradient-based — see §5.2).

**Why this is not double-counting:** in the simplest single-Ganglion case, every scap receives the _same_ `pulse_width`. The differentiation between scaps comes entirely from their _different_ momenta. In the full hierarchical case, `pulse_width` differs by Ganglion (because each Ganglion received a different share after diffusion) but is still the same for every scap _within_ a Ganglion. The within-Ganglion differentiation still comes from momentum. So `pulse_width` and `momentum` operate at different scopes — one is shared across a Ganglion's scaps, the other is per-scap.
...
5. **The measurement circuit converts time-to-threshold into contribution magnitude.** A shorter charge time represents a _larger_ contribution, so the stored value is the post-converted contribution magnitude (effectively `1/T` or equivalent — fixed-time integrated voltage works equally well). By the time the value enters temp SRAM and then momentum SRAM, it already has the semantics of _"how much this wire contributed"_ — larger = more contribution. The downstream distribution math does not need to invert.
6. The 29 measured (converted) values land in temp SRAM.
7. At end of forward pass, each Ganglion's measurement values are:
...
**How these are filled (clarified in 3.3):** each hierarchy level has its own measurement ALU (a wider analog summer + normalizer + ADC, scaled to the level's child count). The Ganglion-level measurement uses 29 measurement caps + DC time-measure (§7.4). Higher levels are cheaper: a block-level measurement sums perhaps 4 Column outputs analog-style, normalizes through an op-amp normalizer, and ADCs the result into 10-12 bit SRAM. The "ADC at the Brainstem interface" in earlier drafts was an oversimplification — every hierarchy level needs its own measurement infrastructure, sized to its child count.

The signals at higher levels are already aggregated and smaller in count, so per-level ADC area is small. Distribution memory at the level is SRAM (sized per §5.7), not analog.
...
> **Open question — mask overlap policy (placeholder).** Independent of where the mask comes from, the masks M₁, M₂, M₃ can be:
>
> - **Mutually exclusive** — disjoint subsets of G's neurons. Cleanest, no gradient collision possible. But G effectively becomes three independent sub-networks under one address space, and the "shared learning across Specialists" story weakens.
> - **Overlapping** — masks share some G-neurons. Real cross-Specialist learning happens in the overlap, but gradient collision returns there.
> - **Partially overlapping with controlled overlap fraction** — the goldilocks zone.
>
> Not resolved in 3.3. For first simulation use mutually exclusive (cleanest baseline). Test overlap variants as a §14-style optimization once H2 is at least minimally validated.
...
Earlier drafts described Cortex as "dense" and Hippocampus as "sparse." We retire that distinction and replace it with the cleaner, more defensible idea: **same Ganglion-and-Column topology, different update cadences.**
...
Implementation cost: **zero new hardware** — just gate the broadcast `update_signal` to Hippocampus on every kth clock (§4.2 per-Column bus). The momentum SRAM accumulates between consolidations.
...
> **Clarification on "same topology" (3.3).** Both Columns have _the same Ganglion topology_ (2-3-3-2) and _the same Column-level composition pattern_ (G + S blocks, etc). They also both have **two output heads** — one for the prediction-loss head, one for the Commissure-to-the-other-Column head. The only difference is which `update_signal` bus they live on (§4.2), which determines their cadence. So "same structure" is true at the Ganglion/Column composition level; the difference is purely in the _temporal control plane_.
...
### 10.5 Hippocampus output — two heads (same structure as Cortex)

Mirroring Cortex, Hippocampus also has two output heads:

- Head A: predicts input. This is the loss signal driving Hippocampus's own learning. At Hippocampus's slow update cadence, the effective target becomes "the _stable consistent pattern_ of input" rather than "the next sample."
- Head B: sends predicted memory state to Cortex via Commissure.

Same structural layout as Cortex — the difference between the two Columns is the update timing (§10.2), not the topology.
...
4. _Scap updates:_ On the broadcast pulse, every scap whose Column-bus is active updates locally per §6.5.

**Per-Column gating (made explicit in 3.3).** Cortex Column, Hippocampus Column, and Commissure each have _their own_ `update_signal` bus, managed independently by the Brainstem (§4.2):

- Cortex's bus is active every clock.
- Hippocampus's bus is active only every k-th clock (consolidation cadence).
- Commissure's bus is active only every M-th clock (M > k, even slower).

On clocks where a Column's bus is inactive, its scaps do not see an update pulse and do not modify their `weight_cap`. Their `momentum SRAM` continues to accumulate measurements from forward passes regardless — momentum filling is forward-pass-driven, not update-pulse-driven.

This means Hippocampus scaps accumulate `k` forward passes worth of contribution into momentum before each update opportunity, while Cortex scaps update on every forward pass. The slow-consolidation effect emerges from this difference alone.

> **Open question (placeholder):** what should the Brainstem's pulse-width be on a Hippocampus consolidation clock? Three candidates: (a) same as Cortex's pulse on every clock — Hippocampus just gets fewer updates but each one is normal-sized, so total weight movement is `1/k` of Cortex's; (b) `k`× larger pulse on consolidation clocks — total weight movement matches Cortex; (c) some accumulated-loss quantity that the Brainstem aggregates over the k-clock window. (a) is the simplest baseline; (b) and (c) require more Brainstem state. Decide empirically.
...
At every backward pass, the sum of shares at every level equals the loss it received, up to finite-precision tolerance ε. In Python simulation, ε should be near floating-point noise. On hardware, ε is bounded by per-level analog precision; the target bound becomes a calibration question per level. Verifiable invariant in simulation.
...
10. **Initialization scheme.** Small uniform magnitudes + random signs + zero momentum (baseline). _Concrete starting values for first simulation:_ weight magnitudes at ~10% of the capacitor range, signs uniformly random (50/50), momentum SRAM at 0 (filled by first forward pass). Compare against Xavier/He analog equivalent (magnitude scaled by `1/√fan_in`) and sparse-init (50% of scaps at zero magnitude, rest at 10%). Initialization sensitivity is a known failure mode of attribution-based methods (§5.4). Concrete numbers are first-pass guesses; tune empirically.
...
**This is also where the Brainstem's full behavior gets validated** — earlier phases use a degenerate Brainstem (just compute loss, send pulse). Phase 4 exercises per-Column gating, autoregressive label management, and consolidation-pulse scheduling.
...
> **Open question — concrete Phase 4 task (placeholder).** "Recall task" is too vague to test against. The architecture is not yet stable enough to commit to a specific task, but the candidates worth considering are:
>
> - **Sequence prediction with partial-cue completion** — memorize a sequence A→B→C→D, then given A→B, complete with C→D. Matches the predictive-coding framing naturally.
> - **Static pattern recognition with noisy input** — memorize K patterns, present noisy versions, identify which K.
> - **Sparse-cue recall** — memorize input-output pairs, present partial input, complete output.
>
> Decide concretely before starting Phase 4 simulation. The choice affects what loss function makes sense (open question 11) and what supervision signal the Brainstem holds.
...
This is **3.3**, not 4.0. Draft4 should be locked, organized for first-time readers, and treated as canonical. This draft is still meant to be re-read with red pen.

**What got resolved between 3.2 and 3.3:**

- The §6.5 update equation now correctly separates `pulse_width` (global LR-like control) from `momentum SRAM` (per-scap contribution accumulator). They are not duplicates; they operate at different scopes (Ganglion-wide vs. per-scap).
- §7.4 measurement clarified — the stored value already represents contribution by the time it lands in temp/momentum SRAM. The time-to-threshold-to-contribution inversion is folded into the measurement circuit.
- §5.6 normalization clarified — it happens analog at the op-amp ALU level, not in binary. Whatever lands in a scap is final-form, ready-to-use.
- §4.2 Brainstem now explicitly manages per-Column update buses with independent cadences. §10.7 mirrors this from the Limbic Loop side.
- §10.2 / §10.5 — both Columns have two output heads (prediction + Commissure). "Same structure" is true at the Ganglion/Column composition level; difference is purely temporal.
- §5.4 fallback paths expanded with Path 0 (noise floor) as cheapest first remediation for dead-weight.
- §8.4 — each hierarchy level has its own measurement infrastructure, not one global ADC.
- Various smaller fixes: §4.4 cross-ref to §14, §16.10 concrete init values, Phase 4 brainstem-validation note.

**What's flagged as placeholder, not yet resolved:**

- Full Brainstem responsibilities (§4.2 note) — likely to grow as the architecture stabilizes.
- Concrete Phase 4 task definition (§17 Phase 4 note).
- SpecialGeneralist mask overlap policy (§9.5 note).
- Hippocampus consolidation pulse-width policy (§10.7 note).
- Op-amp normalizer topology choice (§5.6 final paragraph).

**What's still open and worth pushing on before draft4:**

1. **Will attribution-based learning actually converge on real tasks?** This is H1. Phase 2 simulation answers it. Until that data lands, the architecture is still a bet.
...
3. **Dead-weight collapse.** §5.4 and H7 take this seriously. Path 0 (noise floor) is the first remediation if it appears.
