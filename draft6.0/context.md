...
>
> If you are an AI picking this up cold: read this, then `README.md` (the pivot), `idea/main.ideas.v1.md` (the decisions), `idea/ideas1.md` (the story), and `../docs/essence/the-essence2.md` (the soul — the grown spine; `the-essence.md` is the seed it grew from). The `research/north-star/` folder is the north-star research library (21 files; "phase-2" in its old loose sense = the recurrent brain, **not** the now-done depth Phase 2). The author is **Jakkrin** (GitHub: Jakkarin-Promsee).
...
- *Out (near term):* SPICE, real fabrication, external-benchmark-chasing as the *claim* (small tasks are fine as probes), and **the recurrent thinking brain** (the real north star, *beyond* the numbered phases). *(Status has advanced far past this file's first writing — the phase list once here is superseded. The neocortex organ is **done.** **Stage 1 (Phases 1–6)** built and hardened the cheap SCFF brain — structure → depth round 1 → the objective reframe (contrast supersedes energy-goodness) → characterization → the depth-decay close-out → noise-hardening; **Stage 2 (Phases 7–10)** built the precise namer and validated the whole object — the readout (not GD) → the economy (run live) → the freeze → the honest race. The current per-phase ladder + verdicts are in [`CLAUDE.md`](CLAUDE.md) and [`src/README.md`](src/README.md); the close-out verdict (S14): a substrate-native continual learner, the founding bet refined not inflated. **Phase 11 (the limit map, ran 2026-07-05, S15)** then took that frozen object to real data + scale and answered the red team's "toy data" strike — 8 real arenas × 5 capability channels drawn win/tie/loss/FLOOR, gas a genuine real-drift win over a tuned ER, the identity holding and flooring honestly. Next = the analog-realism (SPICE/PVT) layer; the recurrent brain stays deferred — "simple intelligence first".)*
...
- **Hold the north star gently.** The thinking brain (correctness-as-feeling) — *beyond* the numbered phases (P1–6 = the SCFF cheap brain — structure, depth, characterization, close-out, noise-hardening — all done/frozen; P7–10 = the GD *namer* / Stage 2, all done; P11 = the limit map / real-data validation, done) — is real and deeply his, but deliberately not specced and not in public docs. Don't pull it forward without his direction.

---

## PART 11 — Where things stand & the file map

**State (2026-07-05):** Draft 6.0 spine committed. **Stage 1 — the SCFF cheap brain — is finished and frozen (Phases 1–6):** Phase 1 (structure: the continual win), Phase 2 (depth round 1: energy-goodness can't), Phase 3 (depth round 2: contrast + coordination ADOPTED), Phase 4 (characterization: a substrate-native continual learner, not a static-accuracy competitor; `src/phase4/README.md`), Phase 5 (SCFF close-out: the depth decay SOLVED — sharper InfoNCE temp 0.2 + a short fixed reader read the continual home 8× cheaper; committed cell `SCFFContrastOverlap` temp0.2/w2; `src/phase5/README.md`), Phase 6 (noise-robust SCFF — Scoped-YES: a noise-augmented view hardens the tap channel forward-only and *improves* clean acc, the input-transducer directional residual handed to the read side; committed cell `NoiseAugContrast`; `src/phase6/README.md`). **Stage 2 — the GD *namer* (Phases 7–10) — is DONE:** Phase 7 (the readout — **the 20% is NOT gradient descent**: a closed-form streaming head, RanPAC/SLDA committed, cbrs guard; the spine bends — magnitude-wins), Phase 8 (the economy, run LIVE: a DDM drift-gate meters the 80/20 real — GD-share 0.121, OURS ≈ ½ BP+replay — and the gate is a *safety* mechanism, firing-more-forgets-more; + the why-analog 2×2, 15.4× = 5.4× substrate × 2.9× algorithm), Phase 9 (close & **FREEZE** the maintenance loop: the bulk *rotates but does not forget*, N2 struck, all-tap + CBRS + proto-reanchor + grid-4 cadence; **locked at commit `59d2720`**, worst-BWT −0.028). **Phase 10 — the honest race:** the frozen object, untouched, raced a fair budgeted tuned BP+replay learner (verdicts pinned blind). **The close-out verdict (S14): a substrate-native continual learner, the founding bet refined not inflated** — it ties the fair baseline on the continual home, trails on natural digits (it is continual, not static), and decisively **wins** continual safety (≈10× less forgetting) + noise (every held-out channel); the energy edge over conventional GD is substrate-realized. **The neocortex organ is done.** The discipline that made it honest: **freeze in P9, judge in P10.** **Phase 11 — the limit map (ran 2026-07-05, S15)** then took the frozen object off the toy bench onto real data + scale, drawn as an 8-arena × 5-channel win/tie/loss/FLOOR map: **gas** a genuine real-drift win (prequential 0.789 ≥ a per-arena-tuned ER, beats persistence), real-MNIST safety/order-invariance intact, cross-dataset **type-robust**, the autocorrelated streams (HAR/electric/covtype) **flooring** honestly, the pinned GD-share economy shape confirmed and the analog substrate factor **growing** 5.4→7.4× — the red-team "toy data" strike answered ([`src/phase11/README.md`](src/phase11/README.md)). Next = the analog-realism (SPICE/PVT) layer. The one-file account of the whole frozen two-brain model is `src/phase9-final-architecture.md` (the current head; `src/phase6-final-architecture.md` = the cheap-brain-only snapshot). The research dossier (`research/north-star/`) remains *enrichment for free time*, not the line to walk. Orientation docs (`CLAUDE.md`, `draft6.0/CLAUDE.md`, the `.claude/skills/`, `../docs/draft/project-personal.md`) track draft 6.0; the `draft5.0/src/` simulator (draft-5.1-era) is historical (substrate primitives may carry forward).
...
docs/
  essence/the-essence2.md  — the soul, the grown spine (origin → collapse → return → the ten phases)
  essence/the-essence.md   — the seed soul (kept — what essence2 grew from)
  draft/
