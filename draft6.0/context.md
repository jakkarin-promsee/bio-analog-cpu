...
- *Out (near term):* SPICE, real fabrication, external-benchmark-chasing as the *claim* (small tasks are fine as probes), and **the recurrent thinking brain** (the real north star, *beyond* the numbered phases — Phase 1 structure/done, **Phase 2 depth round 1/done (2026-06-21)**, **Phase 3 depth round 2/done (2026-06-22 — the objective reframe ADOPTED: contrast + cross-layer coordination supersedes energy-goodness; `src/phase3/phase3-summarize.md` + `research/papers/`)**, **Phase 4 characterization/done (2026-06-22 — the capability map vs tuned BP: a substrate-native *continual* learner, not a static-accuracy competitor; WINS continual/nuisance-dim/depth-composition/depth-cheap, TRAILS static difficulty/class-count, honest NEGATIVE on eval-time noise; `src/phase4/phase4-summarize.md`)**, **Phase 5 = optimization** (the sleep/gate maintenance loop, + train-with-noise & natural-data follow-ups) — and the recurrent brain deliberately deferred: "simple intelligence first").
