...
**State (2026-07-03):** Draft 6.0 spine committed. **Stage 1 — the SCFF cheap brain — is finished and frozen (Phases 1–6):** Phase 1 (structure: the continual win), Phase 2 (depth round 1: energy-goodness can't), Phase 3 (depth round 2: contrast + coordination ADOPTED), Phase 4 (characterization: a substrate-native continual learner, not a static-accuracy competitor; `src/phase4/README.md`), Phase 5 (SCFF close-out: the depth decay SOLVED — sharper InfoNCE temp 0.2 + a short fixed reader read the continual home 8× cheaper; committed cell `SCFFContrastOverlap` temp0.2/w2; `src/phase5/README.md`), Phase 6 (noise-robust SCFF — Scoped-YES: a noise-augmented view hardens the tap channel forward-only and *improves* clean acc, the input-transducer directional residual handed to the read side; committed cell `NoiseAugContrast`; `src/phase6/README.md`). **Stage 2 — the GD *namer* (Phases 7–10) — is underway and three-quarters done:** Phase 7 (the readout — **the 20% is NOT gradient descent**: a closed-form streaming head, RanPAC/SLDA committed, cbrs guard; the spine bends — magnitude-wins), Phase 8 (the economy, run LIVE: a DDM drift-gate meters the 80/20 real — GD-share 0.121, OURS ≈ ½ BP+replay — and the gate is a *safety* mechanism, firing-more-forgets-more; + the why-analog 2×2, 15.4× = 5.4× substrate × 2.9× algorithm), Phase 9 (close & **FREEZE** the maintenance loop: the bulk *rotates but does not forget*, N2 struck, all-tap + CBRS + proto-reanchor + grid-4 cadence; **locked at commit `59d2720`**, worst-BWT −0.028). **The disciplined next step is Phase 10 — validation / showcase:** race the *frozen* object (it touches no knob) against a fair BP+replay **accuracy** baseline (P8 settled *energy*) + the multi-domain adaptive gauntlet + a **held-out** noise showcase → an honest Pareto verdict + the Stage-2 close-out. Discipline: **freeze in P9, judge in P10.** The one-file account of the whole frozen two-brain model is `src/phase9-final-architecture.md` (the current head; `src/phase6-final-architecture.md` = the cheap-brain-only snapshot). The research dossier (`research/north-star/`) remains *enrichment for free time*, not the line to walk. Orientation docs (`CLAUDE.md`, `draft6.0/CLAUDE.md`, the `.claude/skills/`, `../docs/draft/project-personal.md`) track draft 6.0; the `draft5.0/src/` simulator (draft-5.1-era) is historical (substrate primitives may carry forward).
...
draft6.0/
  ...
  idea/
    main.ideas.v1.md   — the decision record (N1–N3 + S1–S13)  ← the plan
