# Phase 1 — Result format (the reporting standard)

> The standing **house style** for every Phase-1 run. `README.md` says *what to test*; this file says *what a
> result looks like* so that every run — by you or by a handed-off agent — comes out **comparable, non-toy, and
> deep enough to cite.** Each `experiment-N.md` links here instead of re-describing its figures.
>
...
>
> This is the **floor**. Adapt upward per experiment; never drop below it.

---

## Layer A — House style (the consistency contract)

The enforcement mechanism is **one plotting module, `figures.py`, with one function per catalog figure (Layer
...
| Rule | Locked to |
| --- | --- |
| **Method encoding** | Each method gets ONE colour + linestyle, forever. `Block = solid teal` · `Pure-GD = dashed orange` · `SCFF-only = dotted teal`. World colours fixed: `pos = blue`, `neg = red`. Never reassigned per-plot. |
...
## Layer B — Figure catalog (the "what is each result" backbone)

A small fixed set. An experiment does not invent figures — it **declares which catalog entries it emits.**
...
| **F6** | **Reachability surface** (co-headline — the *shape read*) | final held-out acc as heatmap/surface over (difficulty × size), fixed budget; **block−GD difference map** | the scale-free shape read — *the read we trust most*; a **coarse grid can ride along in any sweep**, not just 1b |
| **F7** | **Backward cost / locality** | **measured** backward op-counts (block vs GD), **annotated** with the structural credit-distance (SCFF = 0, GD = full depth) | *the* structural claim — local & cheap vs global & expensive. Draw the *measured* cost; the distance is a labelled constant, **not** a per-run discovery |
| **F8** | **Boosting shape** (Exp 3) | training error vs #blocks + **theory bound overlay**; companion per-block γ bars | error falls as predicted; **dead-block** watch (γ≈0) |
| **F9** | **Trade curve** (Exp 2a) | held-out acc + backward cost vs SCFF fraction | the 80/20 sweet spot |
| **INV** | **Invariant strip** (every run) | small-multiples: loss-slope · dead-unit % · ceiling-saturation · (Exp3) inter-block drift | health at a glance — same strip every run |
...
are log-spaced). Ablations are **tables, one variable per row, delta vs the GD baseline, IQR in every cell** —
never a bare mean.
...
| --- | --- |
| **exp0** — SCFF + GD alone | F4, F3, F1, F5, INV |
| **Exp 1** — block vs GD | F1 (gap shaded) + F2 scalar, F3, F7, INV (+F5 if Tier-A, +F6 coarse grid; full surface in 1b) |
| **Exp 2a/2b/2c** — inside the block | ablation table + F9 (2a) / acc-vs-GD-depth (2b) / acc + drift (2c), each with F1, INV |
| **Exp 3** — residual-boosting chain | F8, F2, F1, inter-block-drift, INV |
...
> A block-vs-GD difference counts as **real** only if the **IQR bands are disjoint at the final checkpoint**
> *and* the **sign is consistent in ≥ 4 / 5 seeds, paired by seed** (same seed = same stream + same held-out,
> so the pairing is legitimate). Everything else is reported as **"within noise."**

One rule, load-bearing for the honesty of every comparison in the phase.
...
3. **Figures** — the catalog refs, each with a one-line read. **Name both headlines:** F1 (the convergence /
   learning panel) *and* **F6, the reachability shape read** — the surface is the read we trust most, so it
   *co-headlines*; it is not a 1b afterthought.
...
- **Result / figures** = the catalog refs from Layer B (the mandatory set for that experiment, plus any case-by-
  case additions), all drawn through `figures.py` in the Layer-A house style.
