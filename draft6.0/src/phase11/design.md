...
> **Status: 🟡 READY-FOR-REVIEW (final spec 2026-07-04; deep-research folded — §0.4 +
> [`../../research/papers/phase11/README.md`](../../research/papers/phase11/README.md); lab-manager review lands in §8;
> nothing run, nothing committed to `main.ideas`).** A *live spec an agent executes* — the experiment ladder + build
...
2. **"Toy data."** Every P1–P10 number lives on 8×8 digits, 40-D, 10 classes, 1,200 train/domain. Real inside that
   arena; unknown outside it. → **P11.2–P11.4.**
...
| D4 | "any missed box-feasible real-world set?" | **two found**: Wild-Time **Yearbook** (33,431 × 32×32×1, 1930–2013 — real VISION drift, no backbone needed) = the E2 vision leg (declared bonus, gated on download); **MedMNIST/BloodMNIST** (npz, 28×28) = optional famous-data rung; "E2" = P11.3 in the final numbering. Neither gates the phase |
| D5 | "our ER must reproduce field-strength numbers" | anchor source pinned: **van de Ven et al., Nat MI 2022** + the official `GMvandeVen/continual-learning` numbers; the published Split-MNIST ER band is transcribed into the manifest **before** our ER runs (the anchor guard) |
| D6 | "openml IDs: research" | pinned: Fashion-MNIST **40996** · Electricity **151 v1** (45,312 chronological — the "balanced" v2 destroys time order, named as the wrong variant) · Covertype **150/1596** (581k, file-order convention stated) · CIFAR-100 41983 (stretch, verify at bench); MNIST 554 + CIFAR-10 40927 **verified cached on this box** |
...
ER-strong lands in the published Split-MNIST band, transcribed pre-run) · **floor-criterion** (the informative-cell
cut, pinned §2.4 before any run) · **recipe** (Arm B instances carry bit-equal committed hyperparameters; only
{D, W, LUT-cap-by-rule} differ) · **porthole** (ONE seed-frozen projection per source, applied identically to every
learner; bit-identical projected stream across learners — K5 carried) · **no-change** (the persistence baseline
present on every real-world stream) · **budget-report** (per-cell FLOPs/sample + bytes reported; matched where the
comparison claims matched). **Any guard fails → STOP.**
...
sign-consistent in ≥4/5 paired seeds (heavy live cells run 3 seeds, declared; decisive gaps ≤ δ_acc get 9 where
seed-able). **Balanced accuracy** replaces plain accuracy on any stream whose composition table shows >3:1 class
imbalance (gas; others per table). **Every cut is a SHAPE fixed before the run.**
...
- **The FLOOR criterion (the informative-cell cut — C1, pinned by convention BEFORE any run):** a map cell is
  ...
  **or** (b) every raced learner < chance + 2·δ_acc, **or** (c) on a real-world stream, no raced learner beats the
  ...
  map may be all-FLOOR and that is the honest far edge. Fraction-of-ceiling (AA / joint-AA) is annotated in every
  non-FLOOR accuracy cell.
...
- **P11.3 — the real-world streams.** Per stream, the P10 instrument (AA/balanced-AA, AAA, worst-BWT, worst-point
  retention, per-batch STREAM view, energy, GD-share) vs {ER-strong re-tuned, GDumb, naive-BP, no-change,
  proj→namer carried from P11.1}. **(i) the win** iff retention/safety beat ER-strong (real-difference bar) at
  accuracy ≥ ER − δ_acc — deployment story complete; **(ii) the honest scoped claim** iff accuracy loses but
  safety/retention hold ("safer, not stronger" — stated); **(iii) the loss** iff ER matches our safety on nature's
  drift (the synthetic gauntlet overstated the differentiator — say so on the money figure). Gas is the headline;
  a cell that fails the no-change cut renders FLOOR (D2).
- **P11.4 — the cross-dataset gauntlet.** Class-IL 30-way, single growing head. **(i) type-robust** iff worst-point
  all-prev retention ≥ ER-strong − δ_acc AND no dataset-block's classes collapse below chance + 2·δ_acc after the
  stream ends (the LUT/namer keeps all three data types alive); **(ii) type-scoped** iff one block collapses (name
  which and the mechanism — CBRS eviction skew, Gram conditioning, gate saturation at boundaries); **(iii)
  type-broken** iff retention breaks across the board vs ER. Order-invariance read committed (fwd vs rev).
  Invariant watched: namer covariance condition number per sleep (the D-research Q8 risk, instrumented not assumed).
...
- **P11.7 — throughput / stream-rate.** Derived per arena from metered FLOPs/sample (the critical rate where a
  ...
  run on the gas stream. **(i) the regime win** iff a rate exists where ER visibly degrades (skipped samples → AA
  drop) and OURS doesn't — report the rate; **(ii)** if none in the tested range, the economy claim loses its best
  real-time sentence (reported).
...
- **P11.0 — bench: data + loaders + the fair racer per arena + the guards + the two pre-derivations.** ONE
  ...
  tuning harness (P10.0 protocol verbatim, disjoint seed 7, **search grid pinned**), the no-change baseline, the
  balanced-accuracy read, the composition tables, the anchor transcription (van de Ven Split-MNIST ER band;
  Vergara gas numbers). **Pre-derivation 1 (C3):** GD-share-vs-W from the meter formulas — the namer reads the
  ...
  wall-clock pre-estimates for the widest cells (a W=256 sleep-solve is worst-case ~O((L·W)³) ≈ 3e10 FLOPs ≈ tens of
  numpy-seconds *if the code prices a full solve* — the derivation walks the actual code path; sleeps-per-run ×
  solve cost per cell, in the manifest; a cell projected > an evening is descoped/reshaped at bench, not discovered
  mid-run). **Read (incl. failure):** *all green → proceed; any data/order/anchor/guard red →
...
- **P11.2 — the MNIST rung (r1; both arms).** (a) the 5-domain gauntlet — the P10.3 formulas in native 784-space
  (identity / frozen-permutation / rot90 / 3x+4 / +0.6·RMS-of-rung noise) → porthole; **name the correspondence:
...
- **P11.3 — the real-world streams (the headline).** Gas first (Arm A porthole-40 + Arm B native-128; scaler fit on
  ...
  blocks' eval splits). **Read:** win / scoped ("safer, not stronger") / loss per stream (§2.4); FLOOR where
  no-change wins.
- **P11.4 — the cross-dataset gauntlet (the author's ask).** MNIST → Fashion → CIFAR-gray, class-IL 30-way, single
  growing head, per-source frozen projection (per-source unit-RMS on that source's train split only), NTR=1200 /
  NTE=2000 per block, long-randomized blocks, forward + reversed, both arms, 5 seeds. Watch: Gram/covariance
  condition number per sleep; CBRS class-composition of the LUT at stream end (did all three types survive?); gate
  fire pattern at dataset boundaries. **Read:** type-robust / type-scoped (name the block + mechanism) /
  type-broken (§2.4).
...
| **cell state** | win / tie / loss / **FLOOR** per LIMIT-MAP cell (the §2.4 criterion; FLOOR is grey, never counted as a tie) | categorical + number |
...
`pareto_frontier`, `gauntlet_metrics`, the GAUNTLET-STREAM replay machinery (live/seen/cume/sleeps/fires arrays),
`race_bp` (config selector) + `MLP`, `load_digits_split`/`load_cifar_flat`, `CISTREAM_TASKS`,
`NoiseModel`/`infer_noisy`, **all P8/P9/P10 guards**. Committed config: `p8cfg.COMMITTED_LOOP` + `cadence_every=4`,
`HEAD="slda"`, DIM=40 + `[64]*12`.
...
- **`make_arena_stream(arena, seed, *, arm, regime)` (P11.2–P11.5)** — generalizes `make_gauntlet_stream`: the
  5-domain synthetic gauntlet per rung (native-space transforms → porthole; σ_noise = 0.6× the rung's train-split
  input RMS, computed once, frozen — C4), the two switch regimes (rapid-24 / long-randomized [50,70] non-multiples
  of 24), the real-stream wrappers (natural blocks), the cross-dataset stream (per-source projection + unit-RMS,
  class-offset labels 0–29), forward/reversed.
- **`decomp_cells(...)` (P11.1; deliverable A)** — `proj→namer` (the frozen →40 projection straight into
  SLDA+gate+sleep), `no-sleep` variants, `random-frozen-bulk` (committed cell, weights frozen at init); re-uses
  P10 caches for `bulk→namer` (snapshot-then-diff bit-exact). Emits `dbulk_<channel>` arrays.
- **`nochange_baseline(...)` + `nochange_guard` (P11.3; D2)** — predict-previous-label; present on every real
  stream; feeds the FLOOR criterion.
- **`anchor_guard(...)` (P11.2; D5)** — asserts the manifest carries the published Split-MNIST ER band
  (transcribed at bench, source pinned) and that our ER-strong's number lands inside it; red = fix the racer, no
  verdicts bank.
...
`arrays.npz`; `plot_p11.py regen <run-dir>`; `_ckpt.jsonl` fsync'd; commit per rung
(`feat(draft6/phase11): …`).
...
## 8 · The lab-manager review ledger (pre-run review — PENDING; folded here when it lands)

*(This section is filled by the 2–3-agent review (repo-fit · red-team · literature/stats) before the phase runs —
the P10 §8 pattern. Findings land as numbered rows with resolutions; "not changed (checked, defended)" items are
recorded too. No rung runs until this section says READY.)*
