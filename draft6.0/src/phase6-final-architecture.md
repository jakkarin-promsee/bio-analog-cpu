# The SCFF cheap brain — final architecture (v1.1.0, noise-hardened)
...
>
> **What "v1.1.0" means — read this before any claim below.** This freezes the **ideal-math *design*** of the cheap,
> forward-only half of the chip — not its silicon, not its scale, not a benchmark result. Almost every number here is
> from a **behavioral simulation** (numpy, ideal floats: no analog noise, no finite weight-write precision, no ADC
> quantization). The **one exception, and it is the new part**, is Phase 6: it runs the cell against an **honest
> behavioral analog-noise model** (`NoiseModel`, AIHWKit-structured — still *behavioral*, still not SPICE / device
> physics). The tasks are **small and partly synthetic** (built Gaussian generators, 8×8 digits, flat CIFAR). The
> headline depth result is a **representation claim** (per-layer linear probe and a small trained readout) on a **task
> we constructed**, *not* a static-accuracy benchmark win; the headline **noise** result is a **directional-retention
> claim** (does the class *direction* survive the perturbation?), *not* an accuracy-under-noise benchmark. Where a
> part is *designed but not built*, the text says so in line, and §7 has the full ledger. "Finished" means *the design
> is frozen and the cheap-brain research question — including "is it robust enough to trust downstream?" — is
> answered*; the silicon and at-scale validation are explicitly future work.
>
...
> magnitudes). When something below seems surprising, this is usually why. Phase 6 pays this rule a **sixth** time: the
> dangerous kind of noise is *directional* (a coherent shift along the class axis), and the obvious robustness metric —
> per-sample cosine — is **blind** to it, so *retention of the class direction* is the true read.
>
...
> GIM/CLAPP-family **InfoNCE** loss (§2), and Phase 6 added one **noise-augmented view** to it (§2.5). So "SCFF" = the
> frame and the lineage, not Hinton's / the Nature-Comms loss. **Mechanistically the committed cell is closest to
> CLAPP** (local InfoNCE, made online) carrying **SimCLR-style two-view positives** — keeping the "SCFF" name is a
> statement of project *identity*, not a claim of methodological descent from the SCFF paper specifically. We are
> explicit here so no one mistakes it for the paper's exact method. Biological names ("Hippocampus", "Cortex")
> likewise name **circuit elements**, not biology. The committed cell's code name is **`NoiseAugContrast`** (the
> Phase-5 `SCFFContrastOverlap` cell + one noise-augmented InfoNCE view).
...
| **backward cost (the 80/20)** | a **compute-proportion proxy**, measured in the sim as *layers-that-receive-a-weight-update × per-layer op-count* — **not** Joules, charge-cycles, or silicon energy. "~80% SCFF / ~20% GD" is a *share-of-compute* design target, not an energy measurement (the hardware-meaningful meter is **Stage-2** work). |
| **A7** | the eval-time **noise sensitivity** the Phase-4 capability map flagged as the cell's one honest NEGATIVE — the problem Phase 6 exists to fix. |
| **directional retention** | the Phase-6 **noise metric** and the spine's form here: `acc(σ) / acc(0)` measured along the frozen class axis. Robustness = "the class *direction* survived," never Δaccuracy and never (for the enemy that matters) per-sample cosine. |
| **the two doors / the directional residual** | Door A = analog-substrate noise (tap / input / ADC / weight); Door B = the all-noisy data stream. Both share one enemy: a **directional, non-zero-mean** perturbation aligned with the class axis — the *residual* that survives common-mode auto-zeroing. |
| **`NoiseModel` / projected-RMS / linear-readout control** | the honest behavioral analog-noise model (AIHWKit-structured); noise strength measured as **RMS projected onto the class axis** (so i.i.d. and directional are matched on the axis that matters — the tautology-trap guard); the **linear readout on raw input** carried on every noise figure as the *relative*-fragility yardstick (the decisive read is **OURS-vs-linear**). |
...
*implementation*** — reproduce what the brain does with whatever is cheap on this substrate. **And because it is meant
to run on real analog silicon and a never-clean data stream, the cheap brain has been hardened against the one noise
that actually threatens it — a *directional* perturbation of the very class-signal the design exists to protect (§2.5).**
...
**unquantified and deferred to Stage 2**; what Stage 1 establishes is the credit-assignment *structure* (no cross-layer
...
and analog/PVT/full-noise realism are all out of scope for v1.1.0 — Phase 6 adds a *behavioral* noise model (§2.5),
not device physics. This is the ideal-math layer everything physical will
...
patch/timestep to predict), (ii) the Phase-5 **diagnosis and cure of a depth-decay** that GIM's structured setup
never surfaced (§2.3), and (iii) the Phase-6 **noise-hardening of the objective** so the composed class direction
survives an analog substrate (§2.5). When we say "composes with depth," we mean *on flat vectors, with the decay
cured* — the part that is new.
...
for free). The same property is *why* it is sensitive to eval-time noise (A7) — a real, owned tradeoff, and one that
matters more than a footnote: the sensitivity is to **directional** noise, and the substrate's dominant analog
mismatch is *also* directional, so it attacks the very class-direction the whole design exists to preserve. That made
it the architecture's **sharpest open silicon risk** at the end of Phase 5 — **and the problem Phase 6 set out to
harden (§2.5), rather than leave as an open wound.**
...
**Three caveats this bridge does not get to wave away (Stage-2 gates).** (1) **The estimator changes.** Every number in
...
with in-batch negatives; its survival under LUT-streamed negatives is untested**, and is a Stage-2 gate. (2) **The
...
### 2.5 The noise-hardened objective (Phase 6) — the new part

Everything above froze the cheap brain's *learning* in ideal floats. But the cell will run on an **analog substrate**
(drifting charge, offset op-amps, quantizing ADCs) and on a **lifelong, never-clean data stream**, and Phase 4's
capability map had returned one honest NEGATIVE: eval-time noise (**A7**). Phase 6 is the SCFF-side close-out of that
weakness. Its one question is not "does it beat backprop" but **"is the cheap brain robust enough to *trust*
downstream?"**

**Why the fix has to live *here*, in SCFF — not in the readout.** The ordering is forced by **LP-FT**: a trained
readout *preserves* but cannot *manufacture* the robustness a backbone lacks. A7's sensitivity is born in SCFF's own
**per-sample L2 norm** (the very property that wins depth and nuisance-robustness, §2.3), so **no downstream 20% GD
head can rescue it.** The fix is upstream, in the SCFF objective — a Stage-1 problem, which is exactly why noise got
its own phase *before* the GD namer (Stage 2).

**Naming the enemy honestly (Phase 6.0).** Against an honest behavioral noise model (`NoiseModel`, AIHWKit-structured:
uncorrelated mismatch + a directional residual + common-mode + ADC), at matched **projected-RMS on the class axis**, A7
**reproduces and is OURS-specific**: a directional perturbation degrades OURS's class readout **~2× more than a plain
linear-readout control** (directional retention ~0.60 vs the linear ~0.96, **5/5 seeds**), while the substrate's
**common-mode** channel is already **auto-rejected by the same per-sample norm** (measured off/on, identical). The
dominant fragile channels are **tap and input**; weight noise is milder and ADC is fine ≥ 3-bit. The decisive read is
always **OURS-vs-linear**, never OURS alone (every model loses *some* accuracy to noise; the question is whether OURS
is *specifically* fragile — it is).

**The geometry insight — why "retention," not cosine (density ≠ class, a sixth time).** The two noise enemies attack
*different geometry*. **I.i.d.** noise **rotates** each representation — per-sample `cos(clean, noisy)` catches it
(a magnitude perturbation). The dangerous **directional** enemy is a **coherent translation** along the class axis: it
barely rotates any single representation (`cos ≈ 0.97` across all 12 depths) yet slides the *whole cloud* off a fixed
readout. So per-sample cosine is **nearly blind** to the enemy that matters, and the true direction-preservation read
is **retention of the class direction under directional noise** — made direction-specific by the OURS-vs-linear and
dir-vs-iid contrasts. This is the spine paying its sixth bill: the obvious magnitude metric hides the direction
failure.

**The fix: one noise-augmented contrastive view.** `NoiseAugContrast` corrupts **one** of the two InfoNCE views with
the noise model at train time, so the objective is trained to pull a *clean* view and a *noisy* view of the same
sample together — teaching the composed class direction to be **noise-invariant**, entirely forward-only, no backward
pass through the bulk, no stream rewrite (the §2.3 envelope held). Mechanically it is the forward-only surrogate for a
Jacobian/Lipschitz penalty (Bishop's classic result: training with input noise ≈ a Tikhonov penalty, first-order,
without the backward pass).

**The guess the sims overturned — generic, not directional.** The design *predicted* a **directional**-specific
augmentation (aug along the class axis) would be the spine fix. A random-axis control **refuted it**: the ordering is
`iid ≥ randax > dir` — **broad, generic smoothing wins.** The reason is clean and, honestly, more satisfying than the
guess: the directional enemy's axis is **unknown at train time** and **label-free**, so you cannot bet the
augmentation on one direction; invariance in *every* direction (broad iid) generalizes best. We report **generic
Jacobian-smoothing, plainly**, not the directional mechanism we hoped for. (This is the phase's headline honest-science
moment, logged as a result, not buried.)

**What it buys, at full protocol.** On the assembled cell (σ_aug = 1.0, headroom, n=5), the dominant tap-directional
retention lifts **0.817 → 0.865** (paired Δ +0.054, **5/5 by seed**), clean accuracy *improves* (0.526 → 0.555), and
selectivity holds. It clears the phase's hardest gate — the **A6 continual win is kept and slightly improved**
(BWT −0.022 → −0.017, 4/5 paired positive; **a noise-robust representation is also drift-robust**, because features
that move less under noise also move less across a class-incremental stream) — and it is **not a synthetic artifact**
(A7 and the fix reproduce on real data: digits 0.763 → 0.888, CIFAR-flat 0.697 → 0.779).

**Door B — can a class direction form when *every* training sample is corrupted?** The author's Phase-6 insight is that
the model never sees a clean truth set — every datum is one noisy sample banked to the LUT. Measured, the direction
**still forms** from a fully-noisy stream: separability ratio-to-clean **0.93** for a zero-mean stream (Noise2Noise —
the corrupted target shares the clean conditional expectation) and **1.00** for a directional stream. This exposes the
phase's sharp asymmetry: **the directional enemy is lethal *at eval* (a shift off a *fixed* readout) but harmless *in
training* (a consistent shift the representation simply adapts around).** Buffer *purity* (a small-loss filter) was
measured and **not needed** at this noise level (≈ naive averaging) — an *available* higher-noise knob, not a required
one.

**The honest scope of the fix — a *partial* crossing, and a named residual.** Two things keep this a **Scoped-YES**,
not a clean YES: (1) the tap lift is **near, not decisively above,** the pre-registered 0.90 band at the pinned σ\*
(the per-rung optimistic 0.946 sat at the top of a wide IQR; the combined run overrides it to 0.865). (2) The
**input-transducer directional channel is not reached forward-only** (0.733 → 0.696): a generic augmentation hardens
the rep-level (tap) channel it corrupts at train time, but cannot manufacture robustness for a channel it never
touches. That residual — plus any ADC below ~3-bit — is handed to **Stage-2 read-side** as a named brief (calibration
under shift; LP-FT allows the readout to *defend* a channel even though it cannot *create* base robustness). Two
conditional rungs were **skipped on principle**: forward-only weight-flatness (tap ≫ weight, and tap was already
fixed) and re-tuning the per-sample norm itself (**leave the load-bearing norm; harden around it** — the norm is
load-bearing for four P4/P5 properties, and no result paid on *both* the noise and depth axes).

**One line to remember §2.5 by:** the cheap brain now carries a **noise-aware objective** — robustness is built into
the base representation (one corrupted view), not bolted onto the readout — and the enemy it defeats is the
*directional* one the whole design exists to protect against, on its **dominant** channel, with the residual named.
...
classes — and **only that**. It is kept minimal on purpose: **Stage 1 froze the SCFF half (and Phase 6 hardened it
against noise); Stage 2's whole job is to optimize this GD half.** Treat everything here as the *rough* readout —
validated, but not yet tuned.
...
- **A short fixed truncation stack** (the *deployment* default on the continual home) — a from-scratch ~2–3
...
trained by cross-entropy — the one "direction" expense, paid once. (Stage 2's brief sharpens this into a
**direction-pure** readout — cosine / RanPAC — because distance-based heads read a *magnitude*, which the spine
forbids; §8.)
...
> *placement*, and no confidence threshold meets the bar — the exact inverse of the headroom result. So v1.x ships
> a **fixed** reader. (An *oracle* best-per-input selector reaches far higher — a better selector is a Stage-2 /
...
- **The Hippocampus LUT (built as a prototype store).** A deduplicated store of input *prototypes* (winner-take-all
  ...
  is **Complementary Learning Systems**, re-derived from the circuit side. *(Phase 6 measured a **buffer-purity**
  filter on this store — a small-loss cleanliness gate for the never-clean stream — and found it unneeded at the
  tested noise level: an available Stage-2/maintenance knob, §2.5.)*
- **Boosting blocks (validated as a depth *option*; the committed cell ships a single bulk).** The architecture *can*
  chain `[SCFF → GD-checkpoint]` units as **residual boosting blocks** — each a weak corrector of the residual the last
  ...
  and that *reading* block readouts works while *re-injecting* fails. **The committed cell is a single L≈12
  bulk read at the right depth — boosting is a validated capacity option, not part of the shipped cell.** *(Stage 2
  drops it entirely in favor of a single bulk + per-depth read-only heads; §8.)*
- **The threshold learning-gate (DESIGNED, NOT built).** The intended online economy: run cheap local SCFF when the
  ...
  (`\mathcal{L} \ge \theta`, or on a loss-slope plateau — a drift-detection trigger). **It is named and deferred to
  Stage 2** — the committed cell does not run it; the sleep cadence + this gate are the GD-side knobs Stage 2 will tune
...
```
  input x ──▶ [ SCFF bulk: L≈12, width 64, contrast objective, window w=2, τ=0.2, +noise-aug view (σ_aug=1.0) ]
                  │  every layer:  ReLU → per-sample L2-normalize     (forward-only, label-free, local)
                  │  learning:     windowed 2-view InfoNCE, ONE view noise-corrupted; negatives via LUT (chip) / in-batch (sim)
...
**nothing** about forward-inference energy, which is dominated by the SCFF bulk and is not metered here (a Stage-2 job).
- **It composes the depth a task needs, and reads it cheaply** — the §2.3 result: on the synthetic headroom task the
  ...
  not only in §7.
- **It is noise-hardened on its dominant analog channel (Phase 6, scoped).** A directional, class-axis-aligned
  perturbation is the one noise that threatens the design, and a forward-only objective change (one noise-augmented
  contrastive view) substantially hardens the dominant **tap** channel (directional retention 0.817 → 0.865, 5/5;
  near, not decisively above, the 0.90 band), *improves* clean accuracy, and **keeps the continual win** (§2.5). The
  **input-transducer directional residual** is the named exception → Stage-2 read-side. What was Phase 4's sharpest
  open silicon risk is now *answered on the dominant channel and honestly flagged on the residual*.
- **Nuisance-robust** (crosses *above* tuned-BP in high ambient dimension, free, via the per-sample norm) — the same
  per-sample norm whose directional-noise sensitivity Phase 6 hardened. It also **trails on raw static accuracy and
  many-class** problems, *by design* — a structure learner with a cheap namer, not a global error-minimizer. We do not
  claim otherwise.

The through-line, measured: the cell **wins where the substrate lives and trails on static accuracy precisely because
density ≠ class** — it learns structure cheaply, names it cheaply, and now **holds that structure's direction under
the substrate's own noise** — and that is the whole bet.
...
- *Changed / extended:* the augmentation is **random coordinate masking** (`Bernoulli(1−r)`) on a **flat, non-image
  vector**, applied *inside a local window* rather than through one global end-to-end encoder. **Phase 6 adds a
  second, noise-based augmentation** — one view corrupted by the analog `NoiseModel` (§2.5) — so the committed cell's
  positive pair is *masking + noise*, both forward-only.
...
- *Kept:* the finding that **squared goodness is the depth-killer, not the per-sample norm.** This is why we keep the
  mandatory L2 norm (correctness *and* continual-friendliness) while abandoning `‖h‖²` — and why Phase 6 hardened
  *around* the norm rather than replacing it (§2.5).
...
**The Phase-6 noise additions** *(the new lineage)*
- **AIHWKit / hardware-aware analog training (Rasch et al., 2023)** — *Kept:* the **honest behavioral noise model** and
  the finding that **input / tap / ADC noise dominate the accuracy loss, not weight noise**, and that the dangerous
  residual is **directional**. This *set our threat model* (§2.5): tap ≫ weight, and the enemy is the class-axis
  residual that survives common-mode auto-zeroing.
- **Training-with-noise ≈ Tikhonov regularization (Bishop, 1995)** — *Kept:* the first-order equivalence between
  injecting input noise and a **Jacobian/Lipschitz penalty**. This is *why* our noise-augmented view works, and why
  **generic** (broad-iid) smoothing — not a directional-specific augmentation — is the right lever (§2.5).
- **Noise2Noise (Lehtinen et al., 2018)** — *Kept:* the result that a clean target is recoverable from **only noisy
  observations** when the corruption shares the clean conditional expectation (zero-mean, for an L2 objective). This is
  the theory behind **Door B** — the class direction forming from an all-noisy stream (§2.5). *Honest caveat:* InfoNCE
  is not L2-to-a-target, so we treated N2N as a **hypothesis to test**, not a guarantee — and it held (ratio 0.93).
- **LP-FT / backbone-robustness (Kumar et al., 2022)** — *Kept:* the finding that a trained head **preserves but cannot
  manufacture** representation robustness. This is the **ordering argument** that put noise *before* the GD namer: fix
  it in SCFF, or not at all (§2.5).
- **SAM / MeZO / ZOSA (flatness — the toolbox we built but did not ship)** — *Considered, skipped:* forward-only
  weight-flatness (a zeroth-order sharpness pass, the ZO stand-in for SAM's two-pass backprop) was built and
  guard-checked, then **skipped** — tap ≫ weight, and the tap channel was already fixed (§2.5). Kept as an available
  lever if PVT realism later surfaces a weight-channel gap.
...
| **SimCLR** | augmentation-based two-view positive pair under InfoNCE | random **coordinate-masking** + **noise** augmentation on flat vectors, inside a local window |
...
| **AIHWKit / Rasch 2023** | honest analog-noise model; tap≫weight; directional residual | set the Phase-6 **threat model** (the directional class-axis enemy) |
| **Bishop 1995** | input-noise ≈ Jacobian/Tikhonov penalty | why the noise-augmented view works; **generic > directional** smoothing |
| **Noise2Noise** | learn a clean signal from only-noisy data (zero-mean/L2) | the **Door B** theory — direction forms from an all-noisy stream |
| **LP-FT (Kumar 2022)** | a head preserves but can't manufacture backbone robustness | the **ordering** — noise fixed in SCFF, before the GD namer |
| **SAM / MeZO / ZOSA** | flatness; forward-only zeroth-order sharpness | **built, then skipped** (tap≫weight); available weight-channel lever |

---

## 6 · The six-phase journey that produced this model

One model, six phases, one argument. The plan's *destination* held — this is a continual, substrate-native-depth
learner — but two of its *mechanisms* were wrong (deep SCFF; energy-goodness), and each was overturned by a simulation
or a paper, not rescued by tuning. Two more *design guesses* were overturned the same way in Phase 6 (the noise fix is
generic, not directional; the enemy is a translation, not a rotation). The connective tissue is one recurring fault,
**density ≠ class.**
...
4. **Phase 4 — the capability map.** Characterized the adopted cell against a *genuinely-tuned* backprop across seven
   ...
   noise. It flagged two wounds: past ~layer 5 the representation **decays** (→ Phase 5), and the cell is
   **noise-sensitive** (A7 → Phase 6).
5. **Phase 5 — close out the cheap brain's *learning*.** Named the decay (a *direction* failure, **objective-locality,
   not an intrinsic Tunnel**) and cured it with two free levers — a **sharper temperature** (~82% direction, lr-matched)
...
6. **Phase 6 — harden it against noise (this).** A7 is real, **OURS-specific and directional** (2× a linear readout,
   5/5). A forward-only objective change — **one noise-augmented contrastive view** — hardens the dominant tap channel
   (retention 0.817 → 0.865, 5/5), *improves* clean accuracy, and **keeps the continual win** (BWT −0.022 → −0.017),
   confirmed on real data. The sims overturned the design's guess (the fix is **generic**, not directional), and Door B
   (all-data-is-noise) resolved (the direction forms from a fully-noisy stream). Verdict **Scoped-YES**: the dominant
   channel is hardened forward-only; the **input-transducer residual** goes to Stage-2 read-side. **The cheap brain is
   finished *and* noise-hardened.**

---

## 7 · The committed cell, and what's built vs designed

**v1.1.0 cell (exact config).** `NoiseAugContrast` = the Phase-5 `SCFFContrastOverlap` cell **+ one noise-augmented
InfoNCE view**: an **L = 12, width-64** SCFF bulk; objective = windowed two-view InfoNCE with **temperature `τ = 0.2`**,
**coordination window `w = 2`, stride 2** (`w = 4` the bounded depth-closer), **mask rate `r = 0.5`**, **one view
corrupted by `NoiseModel` at `σ_aug = 1.0`** (broad iid — the generic Jacobian-smoothing lever), **batch `B = 32`**
(sim; negatives streamed from the LUT on-chip), mandatory input + inter-layer L2 norm, learning rate `0.03`; **no
residual**. Deployment: a sleep-consolidated **fixed reader** — a short truncation stack (~L2–3) on the continual home;
all-tap for peak accuracy; per-depth heads / boosted blocks for compositional tasks. Forward-only, per-sample (no
running normalization stats). **Methodology:** median over seeds `[42, 137, 271, 314, 1729]`; a difference is "real"
only if the IQR bands are disjoint **and** the sign agrees in ≥4/5 seeds (or, for a within-seed change, the paired Δ
excludes 0 and is ≥4/5 by sign).
...
| **The noise-augmented view (`NoiseAugContrast`, σ_aug=1.0)** | ✅ **built & measured** (Phase 6) — behavioral noise model; **Scoped-YES** |
| **A7 reproduction + the directional threat model** | ✅ **built & measured** (Phase 6) — OURS-specific, 5/5 vs linear |
| **Door B (direction forms from an all-noisy stream)** | ✅ **built & measured** (Phase 6) — zero-mean 0.93, directional 1.00 |
...
| Plasticity-gradient drift slowdown | ✅ measured as a drift fix (Phase 1); a knob for Stage 2 |
| Buffer-purity filter (never-clean stream) | ✅ measured, **not needed** at this noise level (Phase 6) — an available Stage-2 knob |
| Forward-only weight-flatness (ZO sharpness) | ✅ built & guard-checked, **skipped** (Phase 6, tap≫weight) — available weight-channel lever |
| **On-chip negative streaming (CLAPP-style, LUT-sourced)** | 🟡 **designed, NOT built** — sim uses in-batch negatives (§2.4) |
| **The Ch7 threshold learning-gate** | 🟡 **designed, NOT built** → Stage 2 |
| **Mono-forward as an analog circuit; capacitor-leak L2; SPICE/PVT/device noise** | 🟡 designed; behavioral-sim only (ideal floats + a *behavioral* noise model — no SPICE, no device physics) |
...
| Re-tuning the per-sample norm itself | ❌ **skipped** (Phase 6, STOP ②) — the norm is load-bearing; harden around it |
...
replay** (§4 states the fairer same-budget comparison as Stage-2 work). The temperature fix is **null-but-safe** on
flat data with no composable depth (CIFAR-flat needs convolution, out of scope). The **noise** result is a
**directional-retention** claim under an *honest behavioral* analog-noise model — a **partial** band crossing
(0.865, near not decisively above the 0.90 band) on the **dominant tap channel**, continual-safe and
natural-confirmed, with the **input-transducer directional channel + ADC < ~3-bit** as a **named residual** for
Stage-2 read-side. **No result here has been tested under SPICE / device physics, finite weight-write precision, or
true silicon ADC behavior** — the behavioral noise model is the honest stand-in, and the residual is the honest
flag. The architecture's once-sharpest open silicon risk (directional noise attacking the directional class-signal)
is now *answered on the dominant channel*, not eliminated everywhere.
...
is a Stage-2 / silicon-planning decision.

---

## 8 · What v1.1.0 freezes, and what comes next

**Frozen — the cheap brain's *design* is closed:** the SCFF objective (windowed two-view InfoNCE), `τ = 0.2`,
`w = 2`, the per-sample L2 norm, **the noise-augmented view (`σ_aug = 1.0`)**, no residual, the fixed-reader
deployment, and the verdict that this is a **continual** learner that **holds its class direction under the
substrate's directional noise**. These do not move without a result that overturns them. *Frozen means the ideal-math
design (plus a behavioral noise layer) is settled and the cheap-brain research question is answered — it does **not**
mean silicon-validated or benchmark-validated.*

**Stage 2 (Phases 7–9) — optimize the precise (GD) namer.** Every remaining knob is on the ~20% GD back, not the SCFF
front. **P7 — the readout:** learn the class names on the frozen bulk with a **direction-pure** head (cosine / RanPAC
— distance-based heads read a *magnitude*, which the spine forbids); boosting is dropped for a single bulk + per-depth
read-only heads. **P8 — the economy gate + the cost meter:** build the **Ch7 learning-gate** (cheap SCFF most steps,
expensive GD only when the cheap path stalls — a drift-detection trigger) and a **hardware-meaningful** cost meter
(charge cycles / ADC / write energy, replacing the op-count proxy). **P9 — the maintenance loop + the owed baselines:**
tune the sleep cadence, now *readout-aware* (consolidate the extractor-depth features the fixed reader reads), and
settle the owed old-world baselines (the same-budget **BP+replay** continual baseline; the buffer-purity knob and
bulk-drift rate Phase 6 deferred here). Stage 2 also inherits Phase 6's **named residual** — the input-transducer
directional channel + ADC < ~3-bit — as a **read-side** defence (calibration under shift), the complement LP-FT allows
a head to add without manufacturing base robustness.

**The north star (deliberately not specced).** A recurrent, lifelong-learning "thinking" loop where *correctness is a
self-generated feeling* — the organs re-wired to run recurrently at inference (SCFF goodness → the feeling; the LUT →
the queried memory; the gate → think-until-the-feeling-crosses-θ). The parked Phase-5 thread (a better-than-confidence
per-sample selector) comes home here. After Stage 2, the analog-realism layer (SPICE / PVT) opens. *Simple
intelligence first.*
...
**Phase-6 (noise) additions:** AIHWKit / hardware-aware analog training (Rasch et al., 2023) · Training-with-noise ≈
Tikhonov (Bishop, 1995) · Noise2Noise ([1803.04189](https://arxiv.org/abs/1803.04189)) · LP-FT
([2202.10054](https://arxiv.org/abs/2202.10054)) · SAM ([2010.01412](https://arxiv.org/abs/2010.01412)) · MeZO
([2305.17333](https://arxiv.org/abs/2305.17333)) · ZOSA ([2511.09156](https://arxiv.org/abs/2511.09156)).
Full per-paper stories: [`research/papers/`](../research/papers/README.md) (Phase-6 set:
[`research/papers/phase6/`](../research/papers/phase6/README.md)). The arc behind every claim:
[`stage1-report.md`](stage1-report.md) and each `phaseN/README.md`. The decision record:
[`idea/main.ideas.v1.md`](../idea/main.ideas.v1.md). The v1.0.0 ideal-data snapshot this supersedes:
