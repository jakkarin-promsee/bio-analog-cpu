...
*namer*) is underway — Phase 7 (the readout) is DONE; Phase 8 (economy + cost) is the live line, Phase 9 (maintenance)
follows.** One-glance ladder — the depth lives in each `src/phaseN/README.md`, not here:
...
- **Stage 2 continues → P8 · P9** — **P8** the economy gate + the cost meter (price RanPAC vs SLDA; decide *when* the namer fires) · **P9** the maintenance loop + owed baselines (BP+replay, A5, the read-side noise residual). Map: [`src/stage2-design.md`](src/stage2-design.md); the arc so far (P7 done): [`src/stage2-report.md`](src/stage2-report.md).
