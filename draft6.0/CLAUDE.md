...
*namer*) is underway — Phases 7 (readout), 8 (economy + cost), and 9 (maintenance — the loop is now FROZEN) are DONE.
Re-planned 2026-07-02 into four phases; Phase 10 (validation / showcase — the frozen object vs a fair BP+replay
*accuracy* baseline + the multi-domain gauntlet + the noise showcase) is now the live line. Stage 2 = Phases 7–10;
"freeze in P9, judge in P10."** One-glance ladder — the depth lives in each `src/phaseN/README.md`, not here:
...
- **Stage 2 finishes → P10 (live).** **P10 = validation / showcase** ([`src/phase10/design.md`](src/phase10/design.md), rough) — race the **frozen** object (it touches no knob): the existential **BP+replay *accuracy*** fight (P8 settled *energy*), the multi-domain adaptive gauntlet (new/1-back/all-prev acc + cost — the money figure), the noise showcase (a **held-out** battery), **A5** → an honest **Pareto** verdict + the Stage-2 close-out. Discipline: **freeze in P9, judge in P10.** Map: [`src/stage2-design.md`](src/stage2-design.md) §4; arc so far (P7+P8+P9 done): [`src/stage2-report.md`](src/stage2-report.md).
