...
If you are a future AI picking this up: the spec is in `draft5.1-1.md` and `draft5.1-2.md` at the repo root (split only for PDF-export length; no content difference from the unsplit `draft/draft5.1-full.md`). This document is the _story_ of how it got there. Read it if you want to understand why decisions are what they are — and why reopening them mid-simulation would be a mistake.
