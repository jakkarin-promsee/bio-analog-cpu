...
- **Simulation: Stage 1 (Phases 1–5) is complete (June 2026) — the SCFF cheap brain is closed out** (depth decay solved; Phase 6 = GD-side optimization is next). The experiment ladder (1.0 full SCFF → 1.1 full GD → 2.x mix+middle → 3.x sleep → 4.x block chain) in `ideas1.md` has run, and Phase 5 added the depth-readout fix; the results live in `draft6.0/src/` (the report set + ledgers). The old `draft5.0/src/` simulator was built for the *attribution* architecture and is historical (substrate primitives may carry forward).
