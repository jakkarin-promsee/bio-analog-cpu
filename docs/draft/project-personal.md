...
- **Draft 6.0 spine: committed and now measured.** The decisions (N1–N3 + S1–S15) are in `draft6.0/idea/main.ideas.v1.md`; the eleven phases ran and set the open knobs. Nothing was "locked" the way draft-5.1's §22 was; 6.0 earned its commitments from results.
- **Simulation: both stages complete (Phases 1–10, June–July 2026) — the neocortex is built, characterized, frozen, and raced.** Stage 1 (P1–6) built and hardened the SCFF cheap brain (depth cured, noise hardened); Stage 2 (P7–10) built the closed-form namer, ran both brains live, and raced the frozen object against a fair, tuned BP+replay baseline. Verdict **S14**: a substrate-native continual learner (ties the continual home, trails static accuracy, wins safety + noise; energy edge substrate-realized). The results live in `draft6.0/src/` (the report set + ledgers); the whole model in one file is `draft6.0/src/phase9-final-architecture.md`. The old `draft5.0/src/` simulator was built for the *attribution* architecture and is historical (substrate primitives carry forward).
