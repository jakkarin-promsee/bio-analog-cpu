...
### Phase 3 — depth, round 2: the objective reframe (active, 2026-06-21)

**🟡 Active — rough plan drafted (numbers/result-format pending).** Phase 2 proved *energy-goodness* SCFF can't earn depth; Phase 3 tests the one lever it never touched — a different **objective family**. The Phase-3 literature pass (**`draft6.0/ref2/`**) found the depth wall is **intrinsic to the energy objective `G=Σh²`, not to forward-only locality**: Greedy InfoMax / CLAPP — and, historically, greedy layer-wise autoencoder pretraining (Hinton 2006 / Bengio 2007) — are forward-only, unsupervised, *and depth-composing*, because their objective is **information-preserving / predictive**. So Phase 3 swaps SCFF's per-layer goodness from energy → **masked-feature reconstruction** (the decided primary: single-sample, info-preserving, the substrate-native member of the GIM family), with **sibling-contrastive (CLAPP)** as the control, the user's cross-layer **"Direction 1" (OLU) coordination** as a booster rung, and **Mono-Forward** as the supervised-local fallback. The **continual veto is unchanged** — the *new* objective must *re-earn* the Phase-1 win, not inherit it. Plan: **`draft6.0/src/phase3/README.md`**; survey + verdicts: **`draft6.0/ref2/README.md`**. **The old SCFF energy-goodness is broken *for depth* and superseded as the depth path** (its *shallow* continual win, Phase 1, still stands). **Next:** write `phase3/result-format.md`, then run P3.0.
...
| "What's Phase 3 / how do we break the depth wall?" | `draft6.0/src/phase3/README.md` (the plan) + `draft6.0/ref2/README.md` (the survey + verdicts) — **active: the objective reframe** |
