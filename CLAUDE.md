...
- **`draft6.0/research/papers/`** — plain-language stories of the papers behind it (SCFF, Distance-Forward, BoostResNet, BYOL, LLRD; + the Phase-3 reframe papers).
...
**Draft 6.0, Stage 1 complete (Phases 1–4). Phase 5 = optimization is the live line.** One-glance ladder — the depth lives in the per-phase summaries, not here:

- **P1 ✓ structure** — the cell works, and its home is the **continual** regime (a periodic sleep recovers what online backprop catastrophically forgets). Generalizes better than backprop at ~10% backward cost; *not* a deep static-accuracy competitor.
...
- **P5 → optimization** (the live line) — the sleep-cadence + Ch7 learning-gate maintenance loop tuned to *this* cell's measured drift, plus the train-with-noise (hardware-aware) and natural-data follow-ups Phase 4 flagged.

Depth + detail (don't restate it here): **`draft6.0/context.md`** (the cold-start dump) · **`draft6.0/src/stage1-report.md`** (the reader-facing arc) · each **`draft6.0/src/phaseN/phaseN-summarize.md`** + **`RESULTS.md`** (the authoritative per-phase record). For read-heavy sweeps across these, dispatch the **`Explore`** subagent (its own context window; returns only the conclusion) rather than loading them all into this session.

### The build discipline (still governs every rung)
...
- **Test convergence, not theory.** SCFF + GD + boosting are settled; each rung's result is one picture, not an argument. The "so many papers" feeling is the north-star menu (`research/north-star/` 1–6, the dossier) bleeding in — **keep that menu closed.**
...
- **Residual connections** — now confirmed by boosting theory (`research/papers/phase1-2/boostresnet.md`).
...
│   │   ├── survey/                    algorithm survey (the options; attribution = 5.1 history)
│   │   ├── papers/                    paper stories behind 6.0 — Phase 1-3 (scff, deeperforward, distance-forward, boostresnet, byol, llrd; GIM, CLAPP, mono-forward, the reframe)
│   │   └── north-star/                north-star research dossier (21 files + th/ Thai mirror) — beyond the numbered phases, free-time, not the live line
...
├── draft-journey/                     the journey log — every draft 1.0 → 5.1 (the path of thinking; see its README)
...
| "What did Phase 3 (depth round 2) find?" | `draft6.0/src/phase3/phase3-summarize.md` (synthesis) + `RESULTS.md` (ledger) + `draft6.0/research/papers/` (the Phase-3 papers) — **done: the objective reframe ADOPTED (contrast + coordination supersedes energy-goodness)** |
...
| The papers behind a 6.0 decision           | `draft6.0/research/papers/` (paper stories, one per paper — the Phase-1/2 design + the Phase-3 reframe) |
| The algorithm survey / learning-rule zoo   | `draft6.0/research/survey/summary.detail.md`                                     |
| The recurrent-brain / north-star research  | `draft6.0/research/north-star/` (21-file dossier — *beyond* the numbered phases, free-time reading, *not* the live plan) |
